...
def badge(
    ...
    with_uptime = False
    if len(signature) == 10:
        if signature.endswith("-2"):
            with_late = False
        elif signature.endswith("-4"):
            with_uptime = True
    ...
    if with_uptime:
        ...
        for check in q:
            ...
            total_checks += 1
            # Get the uptime for the most recent month
        ...
        if total_checks > 0:
            ...
            # IF there are no ping checks we'll say it's 100% 
            status = "100.0%"
    else:
        for check in q:
            if tag != "*" and tag not in check.tags_list():
    ...
    if fmt == "json":
        ...
        if with_uptime:
            result = {"status": status, "total": total, "grace": grace, "down": down, "uptime_percentage": uptime_percentage}
